...
export default function Transactions({ address }) {
    ...
    if (isLoading) return <div>Loading transactions from {address}</div>; // return <Spinner />
    if (isError) return <div>Error loading transactions from {address}</div>; // return <Error />
